...
// few common paths. Composio may also return asset_url (presigned URL) instead.
...
function extractAssetUrl(result) {
  const candidates = [
    result?.data?.images?.[0]?.asset_url,
    result?.data?.[0]?.asset_url,
    result?.images?.[0]?.asset_url,
    result?.asset_url,
...
async function generateImage(promptText, referenceImagePath, outPath) {
  ...
  // If OpenAI lives under a different Composio entity, swap the URL just for image gen
  const imageConfig = config.composio?.imageGenMcpUrl
    ...
    : config;
  const result = await callTool(imageConfig, toolSlug, args);
  if (result?.error) {
  ...
  if (b64) {
    ...
    return null; // no s3key for b64 path
  } else if (assetUrl) {
    await downloadToFile(assetUrl, outPath);
    // Extract and return the s3key so post-to-instagram can reuse it directly
    try { return new URL(assetUrl).pathname.substring(1); } catch(e) { return null; }
  } else {
    throw new Error(
...
(async () => {
  ...
  for (let i = 0; i < prompts.slides.length; i++) {
    ...
    try {
      const s3key = await withRetry(() => generateImage(fullPrompt, refPhoto, outRaw));
      if (s3key) s3keys[`slide-${i + 1}`] = s3key;
