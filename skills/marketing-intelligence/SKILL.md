...
1. `read_file` `/opt/data/social-marketing/config.json` for timezone, bookingTracking method, platforms enabled.
2. Invoke `terminal`:
   ```bash
   node /opt/hermes/social-marketing-skill/scripts/daily-report.js --config /opt/data/social-marketing/config.json --days 3
   ```
   The script pulls Composio platform stats for each enabled platform (TikTok, Instagram, Facebook), pulls booking data per `bookingTracking.method`, cross-references views↔bookings over a 3-day window, and writes `/opt/data/social-marketing/reports/<YYYY-MM-DD>-daily.md` + appends to `/opt/data/social-marketing/hook-performance.json`.
...
2. `read_file` `/opt/data/social-marketing/config.json` for `country` (gates the local-market query bucket).
...
5. Synthesize findings — you (LLM) do the synthesis, not a script. Write:
   - Structured: `/opt/data/social-marketing/trend-report.json` (schema in [references/trend-queries.md](references/trend-queries.md))
...
4. Write changes to `/opt/data/social-marketing/skill-updates.json` (append-only log: date, what changed, why).
...
1. `read_file` `/opt/data/social-marketing/restaurant-profile.json` for cuisine, location, typicalGuest.
...
6. Synthesize — write:
   - Structured: `/opt/data/social-marketing/competitor-research.json`
   - Narrative: `/opt/data/social-marketing/reports/competitor/<YYYY-MM-DD>.md`
