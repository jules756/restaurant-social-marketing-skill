...
1. Drive sync fails (platform API error, network, expired OAuth) → fall back to the last successful local cache at `/opt/data/social-marketing/photos/`.
...
```
find /opt/data/social-marketing/photos -maxdepth 2 -type f -name '*.jpg' -mtime -7 | head -1
```
...
1. Log error to `/opt/data/social-marketing/errors.json` (append JSON lines).
...
4. Save the post payload (images + caption) to `/opt/data/social-marketing/posts/failed/<timestamp>/` so nothing is lost.
...
2. If still failing → fall back to writing the slide paths and caption to a file: `/opt/data/social-marketing/posts/<timestamp>/telegram-fallback.txt`.
