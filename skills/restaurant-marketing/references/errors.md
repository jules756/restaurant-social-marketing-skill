...
1. Drive sync fails (platform API error, network, expired OAuth) → fall back to the last successful local cache at `~/social-marketing/photos/`.
...
```
find ~/social-marketing/photos -maxdepth 2 -type f -name '*.jpg' -mtime -7 | head -1
```
...
1. Log error to `~/social-marketing/errors.json` (append JSON lines).
...
4. Save the post payload (images + caption) to `~/social-marketing/posts/failed/<timestamp>/` so nothing is lost.
...
2. If still failing → fall back to writing the slide paths and caption to a file: `~/social-marketing/posts/<timestamp>/telegram-fallback.txt`.
